...
def write_image(x, results, color):
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    img = results[int(x[0])]
    cls = int(x[-1])
...
if __name__ == "__main__":
    ...
    for i, batch in enumerate(im_batches):
        ...
        prediction = model.foward(Variable(batch, volatile=True), CUDA)
    ...
    # 在我们绘制边界框之前，让我们将每个边界框的角属性转换到图像的原始尺寸上
    ...
    list(map(lambda x: write_image(x, loaded_ims, color = random.choice(colors)
), output))
